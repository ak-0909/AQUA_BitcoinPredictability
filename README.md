...
How much of daily Bitcoin behaviour is actually predictable from its own price history? This project answers that question honestly — by building a small, interpretable model, then stress-testing it hard enough that a weak signal can't masquerade as a strong one.
...
It's easy to fit a model to crypto prices and report a number that looks good. It's much harder to show whether that number means anything. The design philosophy here is the opposite of the usual "throw 200 features at a deep net" approach:

- **Parsimony over complexity** — 15 economically motivated features, each mapping to a property the data actually exhibits.
- **Validation over the headline metric** — a single in-sample AUC proves nothing; a battery of out-of-sample checks does.
- **Leakage discipline** — strictly trailing features, ratio-based (stationary) inputs, chronological splits, and explicit embargoes so no forward target can leak backwards.

## Data

Daily OHLCV + market cap for Bitcoin, **2010-07-17 → 2024-05-22** (~5,059 rows). The model uses *only* the asset's own market history — no on-chain, macro, or sentiment data — so the question is specifically about self-predictability.
...
```
.
├── BITCOIN_PREDICTABILITY.ipynb   # full pipeline: EDA → features → models → validation
└── README.md
```
...
```bash
pip install numpy pandas matplotlib seaborn scikit-learn statsmodels scipy xgboost catboost shap torch
```

Then open `BITCOIN_PREDICTABILITY.ipynb`, set `CSV_PATH` to your data file, and run the cells top to bottom. The notebook auto-installs `xgboost`, `catboost`, and `torch` if they're missing, so it also runs cleanly in Google Colab.

## Future work

Incorporate on-chain metrics, macroeconomic variables, and sentiment indicators — the natural next sources of signal once the asset's own price history has been shown to be near-exhausted.

---

*Built as a study in disciplined, validation-first quantitative research: the goal is a trustworthy answer, not an impressive-looking one.*
